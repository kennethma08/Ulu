...
namespace WhatsappClient.Controllers
{
    ...
    public class DashboardController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var conversations = await _api.ObtenerConversacionesAsync();           // ConversationSessionDto
            var mensajes = await _api.ObtenerMensajesAsync();                 // MessageDto

            contactos ??= new List<ContactDto>();
            conversations ??= new List<ConversationSessionDto>();
            ...
            int newClients = contactos.Count(c =>
            {
            ...
            foreach (var c in contactos)
            {
                ...
                {
                    var name = GetString(c, "Name", "Nombre", "FullName")
                                 ?? GetString(c, "PhoneNumber", "Telefono", "Celular")
                                 ?? "Cliente";
                    activities.Add(new ActivityItem
                    {
                        Type = "new_client",
                        Title = $"Nuevo cliente: {name}",
                        Subtitle = "",
            ...
            var vm = new DashboardDto
            {
                ...
                MonthlyMessagesLabels = labelsMes,
                MonthlyMessagesValues = valuesMes,
